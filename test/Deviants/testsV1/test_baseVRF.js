...
describe.only("Test base VRF functionality", function() {
    ...
    //     }).catch(err => {
    //         expect(err.message).to.contain("reverted with an unrecognized custom error (return data: 0x71e83137)")
    //     })
    //     expect(await deviantsNFT.numbersRequested()).to.eq(false);
    ...
    it("Try call initChainlink as a user", async() => {
        try {
            await deviantsNFT.connect(add1).initChainlink(1, ethers.utils.formatBytes32String(""), 250000);
        } catch (error) {
            expect(error.message).to.include("Ownable: caller is not the owner");
        }
    });

    it("Try call askRandomNumber as a user", async() => {
        await deviantsNFT.initChainlink(1, ethers.utils.formatBytes32String(""), 250000);
        await hardhatVrfCoordinatorV2Mock.addConsumer(1, deviantsNFT.address)
        ...
        expect(await deviantsNFT.requestIdNr()).to.eq(0)
        try {
            let tx = await deviantsNFT.connect(add1).askRandomNumber();
            let { events } = await tx.wait();
        } catch (error) {
            expect(error.message).to.include("Ownable: caller is not the owner");
        }
