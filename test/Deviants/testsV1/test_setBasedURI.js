...
describe.only("Test setBasedUri function", function() {
    ...
    it("Test setBasedURI with metadata already set", async() => {
        ...
        expect(await deviantsNFT.baseURI()).to.equal("testURI")

        await deviantsNFT.setBasedURI("metadata");
        await deviantsNFT.mintBatch(["0x9c480cd02d8a2ae18de1c6ac96c8fa41c396b146"], 1)
        expect(await deviantsNFT.baseURI()).to.equal("metadata")
    })

    it("Test setBasedURI without URI", async() => {
        const { deviantsNFT, deviantsSilverPass, deviantsDimondPass, deviantsGoldPass, deviantsCrimsonPass, add1, add2, add3, add4, add5, add6, add7, payerAccount, receiver } = await loadFixture(resetState);

        await deviantsNFT.setBasedURI("");
        await deviantsNFT.mintBatch(["0x9c480cd02d8a2ae18de1c6ac96c8fa41c396b146"], 1)
        expect(await deviantsNFT.baseURI()).to.equal("")
    })

    it("Test setBaseURI as a normal user", async() => {
        const { deviantsNFT, deviantsSilverPass, deviantsDimondPass, deviantsGoldPass, deviantsCrimsonPass, add1, add2, add3, add4, add5, add6, add7, payerAccount, receiver } = await loadFixture(resetState);
        let newURI = "newURI"
        let oldURI = await deviantsNFT.baseURI();
        await deviantsNFT.setBasedURI(newURI);
        expect(await deviantsNFT.baseURI()).to.equal(newURI);
        expect(oldURI).not.equal(await deviantsNFT.baseURI())

        let newURIUser = "userTryToSet"
        try {
            await deviantsNFT.connect(add1).setBasedURI(newURIUser);
        } catch (error) {
