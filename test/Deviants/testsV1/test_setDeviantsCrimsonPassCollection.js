...
describe.only("Test setDeviantsCrimsonPassCollection function", function() {
    ...
    it("Set setDeviantsCrimsonPassCollection", async function() {
        ...
        let actualCrimson = await deviantsNFT.deviantsCrimsonPassCollection();
        expect(actualCrimson).to.equal(deviantsCrimsonPass.address);

        await deviantsNFT.setDeviantsCrimsonPassCollection(deviantsSilverPass.address);
        expect(await deviantsNFT.deviantsCrimsonPassCollection()).to.equal(deviantsSilverPass.address);
    })

    it("Try set setDeviantsCrimsonPassCollection as a normal user", async() => {
        const { deviantsNFT, deviantsSilverPass, deviantsDimondPass, deviantsGoldPass, deviantsCrimsonPass, add1, add2, add3, add4, add5, add6, add7, payerAccount, receiver } = await loadFixture(resetState);

        try {
            await deviantsNFT.connect(add1).setDeviantsCrimsonPassCollection(deviantsSilverPass.address);
        } catch (error) {
            expect(error.message).to.include("Ownable: caller is not the owner");
        }
    })

    it("Set setDeviantsCrimsonPassCollection multiple times", async() => {
        const { deviantsNFT, deviantsSilverPass, deviantsDimondPass, deviantsGoldPass, deviantsCrimsonPass, add1, add2, add3, add4, add5, add6, add7, payerAccount, receiver } = await loadFixture(resetState);

        await deviantsNFT.setDeviantsCrimsonPassCollection(deviantsSilverPass.address);
        expect(await deviantsNFT.deviantsCrimsonPassCollection()).to.equal(deviantsSilverPass.address);

        await deviantsNFT.setDeviantsCrimsonPassCollection(deviantsDimondPass.address);
        expect(await deviantsNFT.deviantsCrimsonPassCollection()).to.equal(deviantsDimondPass.address);
    })
})