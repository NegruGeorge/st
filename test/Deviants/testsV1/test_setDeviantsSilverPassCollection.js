...
describe.only("Test setDeviantsSilverPassCollection function", function() {
    ...
    it("Set setDeviantsSilverPassCollection", async function() {
        ...
        let actualSilver = await deviantsNFT.deviantsSilverPassCollection();
        expect(actualSilver).to.equal(deviantsSilverPass.address);

        await deviantsNFT.setDeviantsSilverPassCollection(deviantsSilverPass.address);
        expect(await deviantsNFT.deviantsSilverPassCollection()).to.equal(deviantsSilverPass.address);
    })

    it("Try set setDeviantsSilverPassCollection as a normal user", async() => {
        const { deviantsNFT, deviantsSilverPass, deviantsDimondPass, deviantsGoldPass, deviantsCrimsonPass, add1, add2, add3, add4, add5, add6, add7, payerAccount, receiver } = await loadFixture(resetState);

        try {
            await deviantsNFT.connect(add1).setDeviantsSilverPassCollection(deviantsSilverPass.address);
        } catch (error) {
            expect(error.message).to.include("Ownable: caller is not the owner");
        }
    })

    it("Set setDeviantsSilverPassCollection multiple times", async() => {
        const { deviantsNFT, deviantsSilverPass, deviantsDimondPass, deviantsGoldPass, deviantsCrimsonPass, add1, add2, add3, add4, add5, add6, add7, payerAccount, receiver } = await loadFixture(resetState);

        await deviantsNFT.setDeviantsSilverPassCollection(deviantsSilverPass.address);
        expect(await deviantsNFT.deviantsSilverPassCollection()).to.equal(deviantsSilverPass.address);

        await deviantsNFT.setDeviantsSilverPassCollection(deviantsDimondPass.address);
        expect(await deviantsNFT.deviantsSilverPassCollection()).to.equal(deviantsDimondPass.address);
    })
})