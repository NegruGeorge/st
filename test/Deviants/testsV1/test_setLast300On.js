...
describe.only("Test setLast300On function", function() {
    ...
    it("Test setLast300On", async() => {
        ...
        expect(await deviantsNFT.last300On()).to.equal(false);

        await deviantsNFT.setLast300On(true);
        expect(await deviantsNFT.last300On()).to.equal(true);
    });

    it("Try setLast300On as a normal user", async() => {
        const { deviantsNFT, deviantsSilverPass, deviantsDimondPass, deviantsGoldPass, deviantsCrimsonPass, add1, add2, add3, add4, add5, add6, add7, payerAccount, receiver } = await loadFixture(resetState);

        try {
            await deviantsNFT.connect(add1).setLast300On("Reveal");
        } catch (error) {
