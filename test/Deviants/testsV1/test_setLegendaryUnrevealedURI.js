...
describe.only("Test setLegendaryUnrevealedURI function", function() {
    ...
    it("Test setLegendaryUnrevealedURI", async() => {
        ...
        expect(await deviantsNFT.legendaryUnrevealedURI()).to.equal("legendaryunrevealedURI");

        let newURI = "LEGENDARY"
        await deviantsNFT.setLegendaryUnrevealedURI(newURI);
        expect(await deviantsNFT.legendaryUnrevealedURI()).to.equal("LEGENDARY");
    });

    it("Try setLegendaryUnrevealedURI as a normal user", async() => {
        const { deviantsNFT, deviantsSilverPass, deviantsDimondPass, deviantsGoldPass, deviantsCrimsonPass, add1, add2, add3, add4, add5, add6, add7, payerAccount, receiver } = await loadFixture(resetState);

        try {
            await deviantsNFT.connect(add1).setLegendaryUnrevealedURI("Reveal");
        } catch (error) {
