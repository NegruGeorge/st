...
describe.only("Test setReceiver function", function() {
    ...
    it("Test setReceiver", async() => {
        ...
        expect(await deviantsNFT.receiver()).to.equal(receiver.address)

        let newReceiver = add2.address
        await deviantsNFT.setReceiver(newReceiver);
        expect(await deviantsNFT.receiver()).to.equal(newReceiver);
    });

    it("Try setReceiver as a normal user", async() => {
        const { deviantsNFT, deviantsSilverPass, deviantsDimondPass, deviantsGoldPass, deviantsCrimsonPass, add1, add2, add3, add4, add5, add6, add7, payerAccount, receiver } = await loadFixture(resetState);

        try {
            await deviantsNFT.connect(add1).setReceiver(add3.address);
        } catch (error) {
