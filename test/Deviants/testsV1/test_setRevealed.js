...
describe.only("Test setRevealed function", function() {
    ...
    it("Test setRevealed", async() => {
        ...
        expect(await deviantsNFT.revealed()).to.equal(false);

        await deviantsNFT.setRevealed(true);
        expect(await deviantsNFT.revealed()).to.equal(true);
    });

    it("Try setRevealed as a normal user", async() => {
        const { deviantsNFT, deviantsSilverPass, deviantsDimondPass, deviantsGoldPass, deviantsCrimsonPass, add1, add2, add3, add4, add5, add6, add7, payerAccount, receiver } = await loadFixture(resetState);

        try {
            await deviantsNFT.connect(add1).setRevealed(true);
        } catch (error) {
