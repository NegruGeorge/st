...
describe.only("Test setUnrevealedURI function", function() {
    ...
    it("Test setUnrevealedURI", async() => {
        ...
        expect(await deviantsNFT.unrevealedURI()).to.equal("unrevealedURI");

        let newURI = "reveal"
        await deviantsNFT.setUnrevealedURI(newURI);
        expect(await deviantsNFT.unrevealedURI()).to.equal("reveal");
    });

    it("Try setUnrevealedURI as a normal user", async() => {
        const { deviantsNFT, deviantsSilverPass, deviantsDimondPass, deviantsGoldPass, deviantsCrimsonPass, add1, add2, add3, add4, add5, add6, add7, payerAccount, receiver } = await loadFixture(resetState);

        try {
            await deviantsNFT.connect(add1).setUnrevealedURI("Reveal");
        } catch (error) {
