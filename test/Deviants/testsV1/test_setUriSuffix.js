...
describe.only("Test setUriSuffix function", function() {
    ...
    it("Test setUriSuffix", async() => {
        ...
        let newURI = ".js"
        let oldURI = await deviantsNFT.uriSuffix();
        await deviantsNFT.setUriSuffix(newURI);
        expect(await deviantsNFT.uriSuffix()).to.equal(newURI);
        expect(oldURI).not.equal(await deviantsNFT.uriSuffix())
    });
    ...
    it("Try set suffix as a normal user", async() => {
        ...
        let newURI = ".js"
        try {
            await deviantsNFT.connect(add1).setUriSuffix(newURI);
        } catch (error) {
