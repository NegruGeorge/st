...
describe.only("Test tokenURI function", function() {
    ...
    it("Test tokenURI", async() => {
        ...
        await deviantsNFT.mintBatch([add1.address, add2.address], 18);
        expect(await deviantsNFT.balanceOf(add1.address)).to.equal(18);
        expect(await deviantsNFT.balanceOf(add2.address)).to.equal(18);

        await deviantsNFT.setRevealed(false);
        expect(await deviantsNFT.tokenURI(1)).to.equal("legendaryunrevealedURI")
        expect(await deviantsNFT.tokenURI(16)).to.equal("unrevealedURI")
    })
    ...
    it("Test tokenURI", async() => {
        ...
        await deviantsNFT.setRevealed(true);
        expect(await deviantsNFT.tokenURI(1)).to.equal("testURI1.json");
    })

    it("Test tokenURI without URI", async() => {
        const { deviantsNFT, deviantsSilverPass, deviantsDimondPass, deviantsGoldPass, deviantsCrimsonPass, add1, add2, add3, add4, add5, add6, add7, payerAccount, receiver } = await loadFixture(resetState);
        try {
            await deviantsNFT.tokenURI(123);
        } catch (error) {
