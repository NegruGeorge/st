...
let startPhase1 = 1677416433;

describe("DeviantsCrimsonPass setBasedURI tests", async function() {
    ...
    it("Set basedURI ", async function() {
        ...
        let newURI = "newURI"
        let oldURI = await deviantsCrimsonPass.baseURI();
        await deviantsCrimsonPass.setBasedURI(newURI);
        expect(await deviantsCrimsonPass.baseURI()).to.equal(newURI);
        expect(oldURI).not.equal(await deviantsCrimsonPass.baseURI())
    })

    it("Try setBaseURI as a normal user", async() => {
        const { tree, deviantsCrimsonPass, deviantsGoldPass, deviantsDimondPass, deviantsSilverPass, deployer, add1, add2, add3, add4, add5, add6, add7, add8, payerAccount } = await loadFixture(resetState);
        await time.setNextBlockTimestamp(startPhase1);
        let newURI = "newURI"
        let oldURI = await deviantsCrimsonPass.baseURI();
        ...
        try {
            await deviantsCrimsonPass.connect(add1).setBasedURI(newURIUser);
        } catch (error) {
