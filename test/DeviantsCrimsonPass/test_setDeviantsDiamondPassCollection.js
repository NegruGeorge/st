...
let startPhase1 = 1677416463;

describe("DeviantsCrimsonPass setDeviantsDiamondPassCollection tests", async function() {
    ...
    it("Set setDeviantsDiamondPassCollection", async function() {
        ...
        let actualDimond = await deviantsCrimsonPass.deviantsDiamondPassCollection();
        expect(actualDimond).to.equal(deviantsDimondPass.address);

        await deviantsCrimsonPass.setDeviantsDiamondPassCollection(deviantsSilverPass.address);
        expect(await deviantsCrimsonPass.deviantsDiamondPassCollection()).to.equal(deviantsSilverPass.address);
    })

    it("Try set setDeviantsDiamondPassCollection as a normal user", async() => {
        const { tree, deviantsCrimsonPass, deviantsGoldPass, deviantsDimondPass, deviantsSilverPass, deployer, add1, add2, add3, add4, add5, add6, add7, add8, payerAccount } = await loadFixture(resetState);
        await time.setNextBlockTimestamp(startPhase1);

        try {
            await deviantsCrimsonPass.connect(add1).setDeviantsDiamondPassCollection(deviantsSilverPass.address);
        } catch (error) {
            expect(error.message).to.include("Ownable: caller is not the owner");
        }
    })

    it("Set setDeviantsDiamondPassCollection multiple times", async() => {
        const { tree, deviantsCrimsonPass, deviantsGoldPass, deviantsDimondPass, deviantsSilverPass, deployer, add1, add2, add3, add4, add5, add6, add7, add8, payerAccount } = await loadFixture(resetState);
        await time.setNextBlockTimestamp(startPhase1);
        ...
        await deviantsCrimsonPass.setDeviantsDiamondPassCollection(deviantsDimondPass.address);
        expect(await deviantsCrimsonPass.deviantsDiamondPassCollection()).to.equal(deviantsDimondPass.address);
    })
