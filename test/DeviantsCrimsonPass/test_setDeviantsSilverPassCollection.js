...
let startPhase1 = 1677416523;

describe("DeviantsCrimsonPass setDeviantsSilverPassCollection tests", async function() {
    ...
    it("Set DeviantsSilverPassCollection ", async function() {
        ...
        await deviantsCrimsonPass.setDeviantsSilverPassCollection(deviantsSilverPass.address);
        expect(await deviantsCrimsonPass.deviantsSilverPassCollection()).to.equal(deviantsSilverPass.address);
    })

    it("Try set DeviantsSilverPassCollection as a normal user", async() => {
        const { tree, deviantsCrimsonPass, deviantsGoldPass, deviantsDimondPass, deviantsSilverPass, deployer, add1, add2, add3, add4, add5, add6, add7, add8, payerAccount } = await loadFixture(resetState);
        await time.setNextBlockTimestamp(startPhase1);

        try {
            await deviantsCrimsonPass.connect(add1).setDeviantsSilverPassCollection(deviantsSilverPass.address);
        } catch (error) {
            expect(error.message).to.include("Ownable: caller is not the owner");
        }
    })

    it("Set DeviantsSilverPassCollection multiple times", async() => {
        const { tree, deviantsCrimsonPass, deviantsGoldPass, deviantsDimondPass, deviantsSilverPass, deployer, add1, add2, add3, add4, add5, add6, add7, add8, payerAccount } = await loadFixture(resetState);
        await time.setNextBlockTimestamp(startPhase1);
        ...
        await deviantsCrimsonPass.setDeviantsSilverPassCollection(deviantsDimondPass.address);
        expect(await deviantsCrimsonPass.deviantsSilverPassCollection()).to.equal(deviantsDimondPass.address);
    })
