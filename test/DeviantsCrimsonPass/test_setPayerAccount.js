...
let startPhase1 = 1677416611;

describe("DeviantsCrimsonPass setPayerAccount tests", async function() {
    ...
    it("Set payerAccount as owner", async function() {
        ...
        let beforePayer = await deviantsCrimsonPass.payerAccount();
        await deviantsCrimsonPass.setPayerAccount(add2.address.toLowerCase());
        let afterPayer = await deviantsCrimsonPass.payerAccount();
        expect(beforePayer.toLowerCase()).not.equal(afterPayer.toLowerCase());
        expect(afterPayer.toLowerCase()).to.equal(afterPayer.toLowerCase());
    })

    it("Try to setPayerAccount as a user", async() => {
        const { tree, deviantsCrimsonPass, deviantsGoldPass, deviantsDimondPass, deviantsSilverPass, deployer, add1, add2, add3, add4, add5, add6, add7, add8, payerAccount } = await loadFixture(resetState);
        await time.setNextBlockTimestamp(startPhase1);

        try {
            await deviantsCrimsonPass.connect(add2).setPayerAccount("0x9c480cd02d8a2ae18de1c6ac96c8fa41c396b146");
        } catch (error) {
