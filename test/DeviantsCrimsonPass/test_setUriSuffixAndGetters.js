...
let startPhase1 = 1677416640;

describe("DeviantsCrimsonPass setUriSuffix tests", async function() {
    ...
    it("Set newUriSuffix", async function() {
        ...
        let newURI = ".js"
        let oldURI = await deviantsCrimsonPass.uriSuffix();
        await deviantsCrimsonPass.setUriSuffix(newURI);
        expect(await deviantsCrimsonPass.uriSuffix()).to.equal(newURI);
        expect(oldURI).not.equal(await deviantsCrimsonPass.uriSuffix())
    })

    it("Try to set newUriSuffix as a user", async() => {
        const { tree, deviantsCrimsonPass, deviantsGoldPass, deviantsDimondPass, deviantsSilverPass, deployer, add1, add2, add3, add4, add5, add6, add7, add8, payerAccount } = await loadFixture(resetState);
        await time.setNextBlockTimestamp(startPhase1);
        let newURI = ".js"
        try {
            await deviantsCrimsonPass.connect(add1).setUriSuffix(newURI);
        } catch (error) {
            expect(error.message).to.include("Ownable: caller is not the owner");
        }
    })

    it("Test getPhause1Status", async() => {
        const { tree, deviantsCrimsonPass, deviantsGoldPass, deviantsDimondPass, deviantsSilverPass, deployer, add1, add2, add3, add4, add5, add6, add7, add8, payerAccount } = await loadFixture(resetState);
        await time.setNextBlockTimestamp(startPhase1);
