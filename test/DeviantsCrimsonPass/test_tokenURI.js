...
let startPhase1 = 1677416670;

describe("DeviantsCrimsonPass tokenURI tests", async function() {
    ...
    it("Set newUriSuffix", async function() {
        ...
        try {
            await deviantsCrimsonPass.tokenURI(122222);
        } catch (error) {
    ...
    it("Test tokenURI with metadata already set", async() => {
        ...
        await deviantsCrimsonPass.setBasedURI("metadata");
        await deviantsCrimsonPass.mintBatch(["0x9c480cd02d8a2ae18de1c6ac96c8fa41c396b146"], 1)
        expect(await deviantsCrimsonPass.tokenURI(1)).to.equal("metadata1.json")
    })

    it("Test tokenURI without URI", async() => {
        const { tree, deviantsCrimsonPass, deviantsGoldPass, deviantsDimondPass, deviantsSilverPass, deployer, add1, add2, add3, add4, add5, add6, add7, add8, payerAccount } = await loadFixture(resetState);
        await time.setNextBlockTimestamp(startPhase1);

        await deviantsSilverPass.setBasedURI("");
        await deviantsSilverPass.mintBatch(["0x9c480cd02d8a2ae18de1c6ac96c8fa41c396b146"], 1)
        expect(await deviantsSilverPass.tokenURI(1)).to.equal("")
    })
});