...
describe("DeviantsCrimsonPass globalPause tests", async function() {
    ...
    it("Try to withdraw as a user", async function() {
        ...
        try {
            await deviantsCrimsonPass.withdraw();
        } catch (error) {
            expect(error.message).to.include("DMPC: contract balance is zero");
        }
    })
